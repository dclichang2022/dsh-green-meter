...
export function GreenMeterDock({ useProjection, placement, t }: GreenMeterDockProps) {
  ...
  return (
    <div className={css.root} data-green-meter="live">
      <button className={css.trigger} onClick={() => { panelActions.toggle() }} data-green-meter="toggle">
      ...
      {placement === 'popover' && open
        ? <PopoverPanel meter={meter} t={t} onClose={() => { panelActions.close() }} />
        : null}
    </div>
  )
}
