...
import {
  H20_MODEL_PROFILES,
  H20_PROFILES,
  H20_PROXY_COEFFICIENTS,
} from './h20-profiles.generated.ts'
...
function h20Profile(sourceKey: string, label: string): ModelProfile {
...
export const MODEL_PROFILES: Readonly<Record<ProfileKey, ModelProfile>> = {
  'qwen3-4b-instruct': {
 ...
 */
export function resolveProfile(key: string | undefined): ModelProfile {
  if (key !== undefined && key in MODEL_PROFILES) return MODEL_PROFILES[key as ProfileKey]
 ...
 */
export function selectProfile(modelId: string | undefined): ModelProfile {
  if (modelId === undefined) return MODEL_PROFILES.proxy
