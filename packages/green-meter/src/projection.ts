...
import { carbonGrams, estimateStep } from './estimator.ts'
import { resolveProfile, type ProfileKey } from './profiles.ts'
import {
  GREEN_METER_MAX_STEPS,
  GREEN_METER_MAX_TURNS,
  type GreenMeterProjection,
  type GreenMeterStep,
  type GreenMeterTurn,
} from './types.ts'
...
const EMPTY: GreenMeterFoldState = {
  requests: 0,
  inputTokens: 0,
  outputTokens: 0,
  reasoningTokens: 0,
...
function appendTurn(turns: readonly GreenMeterTurn[], turn: number, energyJ: number, carbonG: number): GreenMeterTurn[] {
  const last = turns.at(-1)
  if (last !== undefined && last.turn === turn) {
    const updated: GreenMeterTurn = {
      turn: last.turn,
      steps: last.steps + 1,
  ...
  const entry: GreenMeterTurn = { turn, steps: 1, energyJ: round4(energyJ), carbonG: round4(carbonG) }
...
function appendStep(steps: readonly GreenMeterStep[], turn: number, step: number, input: number, output: number, energyJ: number, carbonG: number): GreenMeterStep[] {
  const entry: GreenMeterStep = {
    turn,
    step,
    inputTokens: input,
    outputTokens: output,
...
export function greenMeterProjectionDefinition(
  profileKey: ProfileKey,
  carbonFactor: number,
  budgetJ = 0,
  priceCnyPerKwh = 0.56,
): ProjectionDefinition<'greenMeter', GreenMeterFoldState | null> {
  ...
  return {
    ...
    view: (state): GreenMeterProjection | null => state === null ? null : {
