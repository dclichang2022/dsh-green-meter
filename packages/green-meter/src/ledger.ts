...
export class SessionTracker {
  constructor(private readonly profileKey: ProfileKey, private readonly carbonFactor: number) {}
...
export async function scanLedger(path: string): Promise<StepRecord[]> {
  let text: string
  try {
  ...
  for (const line of text.split('\n')) {
    ...
    try {
      ...
      if (
        ...
        && (record as StepRecord).schema_version === STEP_SCHEMA_VERSION
        && (record as StepRecord).method === 'token-profile-estimate'
      ) {
        rows.push(record as StepRecord)
      }
    } catch {
...
export function computeTotals(rows: readonly StepRecord[]): SessionTotals {
  let requests = 0
  let inputTokens = 0
...
function turnBreakdown(rows: readonly StepRecord[]): Map<number, { steps: number, energyJ: number, carbonG: number }> {
  const byTurn = new Map<number, { steps: number, energyJ: number, carbonG: number }>()
  for (const row of rows) {
...
export function formatSessionReport(
  ...
  const lines: string[] = ['Green Meter — energy & carbon estimate', '']
